...
<!-- 38. 点击其他的分配权限按钮, 遗留了上一次的id数据, 要监听对话框关闭事件 @close -->
    ...
    <el-dialog title="分配权限" :visible.sync="setRightDialogVisible" width="30%" @close="setRightsDialogClosed">
...
export default {
  data() {
    return {
      ...
      //33. 默认选中的节点的id值数组
    //   defKeys:[ 105, 116 ],
      //经过测试, 只需要获取权限3的所有id, 并放到数组里,所以要定义方法
       defKeys:[],
       //储存角色id
       roleId:''
  ...
  methods: {
    ...
    async showSetRightDialog(role) {
       ...
       //37.调用递归函数, 在showSetRightDialog传参scope.row, role形参接收
    ...
    //41. 点击分配角色授权
    async allotRights(){
        // 根据传参分析, 需要传一二三级权限的id, 需要调用tree组件的两个方法, 而获取, 在el-tree标签中设置ref="treeRef"获取引用对象, 利用扩展运算符合并
        const keys = [
